...
namespace FluentTime.Tests
{
	...
	public class NumberExtensionTest
	{
		...
		public void Creates_TimeSpans_readably_with_doubles ()
		{
			...
			Assert.That(1.5.Milliseconds(), Is.EqualTo(new TimeSpan((long)(TimeSpan.TicksPerMillisecond * 1.5))));
		}
		...
		{
			Assert.That(2.Days(3.Hours(4.Minutes(15.Seconds(5.Milliseconds())))), 
			            Is.EqualTo(new TimeSpan(2, 3, 4, 15, 5)));

			Assert.That(2.Milliseconds(3.Ticks(4.Ticks())), Is.EqualTo(new TimeSpan((2 * TimeSpan.TicksPerMillisecond) + 7)));

			Assert.That(1.Day(1.Hour(1.Minute(1.Second(1.Millisecond(1.Tick()))))), 
			            Is.EqualTo(24.Hours(60.Minutes(60.Seconds(1001.Milliseconds()))) + 1.Tick()));
		}
	}
}
